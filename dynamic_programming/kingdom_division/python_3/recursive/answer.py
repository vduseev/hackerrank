...
def scan_graph_from_input():
    ...
    graph = {
        city_id: {
            CHILDREN: set(),  # set of adjacent cities
    ...
    for i in range(n - 1):
        ...
        if road_start != road_end:  # Avoid self loops
            graph[road_start][CHILDREN].add(road_end)
            graph[road_end][CHILDREN].add(road_start)
...
def T(m, g, node, color, is_parent_same_color):
    ...
    # Consider usual nodes with parents and children here
        ...
        # neighbor nodes set.
        child_nodes = g[node][CHILDREN] - {g[node][PARENT]}
